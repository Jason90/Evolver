...
{
    /// <summary>
    /// 所有继承 <see cref="BaseEntity"/> 的实体：当前 <see cref="ITenantContext.TenantId"/> + <see cref="ITenantContext.OrgId"/> + 未软删除。
    /// 跨租户/全组织查询请使用 <c>IgnoreQueryFilters()</c>。
    /// </summary>
    private void ApplyTenantOrgSoftDeleteFilters(ModelBuilder modelBuilder)
    {
        foreach (var entityType in modelBuilder.Model.GetEntityTypes())
        {
            ...
            var body = Expression.AndAlso(Expression.AndAlso(tenantEq, orgEq), notDeleted);
    ...
    /// <summary>
    /// <see cref="Tenant"/> 不按 <see cref="BaseEntity"/> 建模，单独过滤：当前租户主键 + 未软删除。
    /// </summary>
    private void ApplyTenantRowQueryFilter(ModelBuilder modelBuilder)
    {
        ...
        var notDeleted = Expression.Equal(Expression.Property(parameter, nameof(Tenant.IsDeleted)), Expression.Constant(false));
